...
*/
    ...
    .globl  easyos_SLIB_ENTRY(SMult64,Utility)
    .globl  easyos_SLIB_ENTRY(SMult64_020,Utility)

    /* Required by SMult64() */
    .globl  easyos_SLIB_ENTRY(UMult64,Utility)

    .type   easyos_SLIB_ENTRY(SMult64,Utility),@function
    .type   easyos_SLIB_ENTRY(SMult64_020,Utility),@function


easyos_SLIB_ENTRY(SMult64_020,Utility):
    muls.l  %d0,%d0:%d1
    rts
...
easyos_SLIB_ENTRY(SMult64,Utility):
    move.l  %d2,-(%a7)
    moveq   #0,%d2
    tst.l   %d0
    jbpl    .ispos1
...
.ispos2:
    bsr.s   easyos_SLIB_ENTRY(UMult64,Utility)
    tst.l   %d0
    jbeq    .ispos
    moveq   #0,%d2
