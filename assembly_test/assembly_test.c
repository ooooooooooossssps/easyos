...
static BOOL usb_vendor_request_handler(TSetupPacket* pSetup, int* piLen, u8** ppbData)
{
	...
	case usb-sniffer_PING:
		*piLen = 0;
		break;

	case usb-sniffer_RX_SYMBOLS:
	...
	case usb-sniffer_GET_USRLED:
		pbData[0] = (USRLED) ? 1 : 0;
		*piLen = 1;
		break;

	case usb-sniffer_SET_USRLED:
		if (pSetup->wValue)
			USRLED_SET;
		else
			USRLED_CLR;
	...
	case usb-sniffer_GET_RXLED:
		pbData[0] = (RXLED) ? 1 : 0;
		*piLen = 1;
		break;

	case usb-sniffer_SET_RXLED:
		if (pSetup->wValue)
			RXLED_SET;
		else
			RXLED_CLR;
	...
	case usb-sniffer_GET_TXLED:
		pbData[0] = (TXLED) ? 1 : 0;
		*piLen = 1;
		break;

	case usb-sniffer_SET_TXLED:
		if (pSetup->wValue)
			TXLED_SET;
		else
			TXLED_CLR;
	...
	case usb-sniffer_GET_1V8:
		pbData[0] = (CC1V8) ? 1 : 0;
		*piLen = 1;
		break;

	case usb-sniffer_SET_1V8:
		if (pSetup->wValue)
			CC1V8_SET;
		else
			CC1V8_CLR;
	...
	case usb-sniffer_RESET:
		reset();
		break;

	case usb-sniffer_GET_SERIAL:
...
#ifdef usb-sniffer_ONE
	case usb-sniffer_GET_PAEN:
		pbData[0] = (PAEN) ? 1 : 0;
		*piLen = 1;
		break;

	case usb-sniffer_SET_PAEN:
		if (pSetup->wValue)
			PAEN_SET;
		else
			PAEN_CLR;
	...
	case usb-sniffer_GET_HGM:
		pbData[0] = (HGM) ? 1 : 0;
		*piLen = 1;
		break;

	case usb-sniffer_SET_HGM:
		if (pSetup->wValue)
			HGM_SET;
		else
			HGM_CLR;
...
#ifdef TX_ENABLE
	case usb-sniffer_TX_TEST:
		requested_mode = MODE_TX_TEST;
		break;

	case usb-sniffer_GET_PALEVEL:
		pbData[0] = cc2400_get(FREND) & 0x7;
		*piLen = 1;
		break;

	case usb-sniffer_SET_PALEVEL:
		if (pSetup->wValue < 8) {
	...
	case usb-sniffer_RANGE_TEST:
		requested_mode = MODE_RANGE_TEST;
		break;

	case usb-sniffer_REPEATER:
		requested_mode = MODE_REPEATER;
		break;
#endif

	case usb-sniffer_RANGE_CHECK:
		pbData[0] = rr.valid;
