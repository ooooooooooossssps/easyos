...
#define sc		FirstArg+8
	...
	.text
	.align	2
	.globl	easyos_CDEFNAME(linux_sighandler)
	.type	easyos_CDEFNAME(linux_sighandler),@function
easyos_CDEFNAME(linux_sighandler):
	/* save regs */
	movem.l	%a5-%a6,-(%sp)
#ifdef __PIC__
	...
	move.l	signum(%sp),-(%sp)
#ifdef __PIC__
	bsr.l	easyos_CSYMNAME(call_inthandlers)@PLTPC
#else
	jbsr	easyos_CSYMNAME(call_inthandlers)
#endif
	addq.w	#4,%sp
#ifdef __PIC__
...
#ifdef TEST
#  ifdef __PIC__
	bsr.l	easyos_CSYMNAME(Dispatch)@PLTPC
#  else
	jbsr	easyos_CSYMNAME(Dispatch)
#  endif
#else
#  if !UseRegisterArgs
	move.l	%a6,-(%sp)
