...
/* Define this wrapper here, before loading easyos headers,
 * so that the defines in <easyos/system.h> do not
...
static char *check_header(struct elfheader *eh)
{
    ...
    if (eh->type != ET_REL || eh->machine != easyos_ELF_MACHINE)
               ...
               void **kick_end, kernel_entry_fun_t *kernel_entry, struct ELF_ModuleInfo **kernel_debug)
{
    ...
    for (n = FirstELF; n; n = n->Next)
    {
        ...
        for (i=0; i < n->eh->shnum; i++)
        {
            ...
            if ((sh[i].type == easyos_ELF_REL) && sh[sh[i].info].addr)
            {
