...
easyos_LH1(void, Cause,
    easyos_LHA(struct Interrupt *, softint, A1),
	...
	if( softint->is_Node.ln_Type != NT_SOFTINT )
	{
		...
		if (!softblock)
		{
...
#endif
			{
				...
				if (iv->iv_Code)
				{
					  easyos_UFC5(void, iv->iv_Code,
					      easyos_UFCA(ULONG, 0, D1),
					      easyos_UFCA(ULONG, 0, A0),
					      easyos_UFCA(APTR, NULL, A1),
					      easyos_UFCA(APTR, iv->iv_Code, A5),
					      easyos_UFCA(struct ExecBase *, SysBase, A6)
					  );
				}
			}
		}
...
easyos_UFH5(void, SoftIntDispatch,
    easyos_UFHA(ULONG, intReady, D1),
    easyos_UFHA(struct Custom *, custom, A0),
    easyos_UFHA(IPTR, intData, A1),
    easyos_UFHA(IPTR, intCode, A5),
		...
		for(i=0; i < 4; i++)
		{
			while( (intr = (struct Interrupt *)RemHead((struct List *)&SysBase->SoftInts[i])) )
			{
		 		...
		 		/* Call the software interrupt. */
		  		easyos_UFC3(void, intr->is_Code,
				   easyos_UFCA(APTR, intr->is_Data, A1),
				   easyos_UFCA(APTR, intr->is_Code, A5),
				   easyos_UFCA(struct ExecBase *, SysBase, A6));
			}
		}
