...
#warning TODO: SysBase->ChkSum=.....
	...
	if (NULL == (arm_SP_User =(ULONG *)AllocMem(easyos_STACKSIZE,MEMF_PUBLIC)))  {
		do {} while(1);
	}
	arm_SP_User = (ULONG *)(((ULONG)arm_SP_User) + easyos_STACKSIZE);
	 ...
	 */
#if 0
	if (NULL == AllocAbs(easyos_STACKSIZE, 
	                     (APTR)(initial_ssp+sizeof(ULONG)-easyos_STACKSIZE))) {
...
//	Init_Traps();
	...
	if (NULL == (arm_SP_IRQ =(ULONG *)AllocMem(easyos_STACKSIZE,MEMF_PUBLIC)))  {
		do {} while(1);
	}
	arm_SP_IRQ = (ULONG *)(((ULONG)arm_SP_IRQ) + easyos_STACKSIZE - sizeof(ULONG));
	D(bug("Now setting IRQ Stackpointer! %x\n",arm_SP_IRQ));
	set_sp_mode(arm_SP_IRQ, MODE_IRQ);

	if (NULL == (arm_SP_FIQ =(ULONG *)AllocMem(easyos_STACKSIZE,MEMF_PUBLIC)))  {
		do {} while(1);
	}
	arm_SP_FIQ = (ULONG *)(((ULONG)arm_SP_FIQ) + easyos_STACKSIZE - sizeof(ULONG));
	D(bug("Now setting FIQ Stackpointer! %x\n",arm_SP_FIQ));
	set_sp_mode(arm_SP_FIQ, MODE_FIQ);

	if (NULL == (arm_SP_Abort =(ULONG *)AllocMem(easyos_STACKSIZE,MEMF_PUBLIC)))  {
		do {} while(1);
	}
	arm_SP_Abort = (ULONG *)(((ULONG)arm_SP_Abort) + easyos_STACKSIZE - sizeof(ULONG));
	D(bug("Now setting Abort Stackpointer! %x\n",arm_SP_Abort));
	set_sp_mode(arm_SP_Abort, MODE_ABORT);

	if (NULL == (arm_SP_Undef =(ULONG *)AllocMem(easyos_STACKSIZE,MEMF_PUBLIC)))  {
		do {} while(1);
	}
	arm_SP_Undef = (ULONG *)(((ULONG)arm_SP_Undef) + easyos_STACKSIZE - sizeof(ULONG));
...
/*
 * The following function will be executed whan easyos is in user mode
 */
static void main_init_cont(void)
{
